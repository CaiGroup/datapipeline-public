...
class Decoding:
    def __init__(self, data_dir, position, decoded_dir, locations_dir, position_dir, barcode_dst, barcode_src,
                 ...
                 num_zslices, segmentation, decode_only_cells, labeled_img, num_wav, lvf, zvf, lwvf,
                 Hpath, start_time):
    ...
    def run_decoding_across(self):
        ...
        if not self.seg:
            ...
            decoding_non_parallel.decoding(barcode_file_path, locations_path, self.decoded_dir, allowed_diff=self.allowed_diff,
                              min_seeds=self.min_seeds)

        else:

            print('Shape in Decoding Class: ' + str(self.labeled_img.shape))
            decoding_parallel.decoding(barcode_file_path, locations_path, self.labeled_img, self.decoded_dir,
                                       self.allowed_diff, \
    ...
    def run_decoding_across_2d(self):
        ...
        for z in range(self.num_zslices):
            ...
            if not self.seg:
                decoding_non_parallel.decoding(barcode_file_path, locations_path_z, decoding_dst_z, allowed_diff=self.allowed_diff,
                                  min_seeds=self.min_seeds)
        ...
        for channel in self.decoding_individual:
            ...
            for z in range(self.num_zslices):
                ...
                if not self.seg:
                    ...
                    # --------------------------------------------------------------------
                    decoding_non_parallel.decoding(
    ...
    def run_decoding_with_previous_locations(self):
        ...
        previous_locations_decoding.decoding(barcode_dst, mat_file_path, self.decoded_dir,
                                             allowed_diff=self.allowed_diff, min_seeds=self.min_seeds)
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == 'debug_decoding_class_indiv_takei':
        labeled_img = tf.imread(
            '/groups/CaiLab/analyses/nrezaee/2020-08-08-takei/takei_strict_8/MMStack_Pos0/Segmentation/labeled_img_post.tif')
