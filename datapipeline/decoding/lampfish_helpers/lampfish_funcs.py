...
def get_3_by_3(tiff_3d, df_locs, offset, hyb, new_col_name):
    ...
    for i in range(df_locs.shape[0]):
        ...
        if round(z_s[i]) >= z_max - 2:
            square_3 = tiff_3d[z_max - 2:z_max - 1, round(y_s[i] - 1):round(y_s[i] + 2),
                       round(x_s[i] - 1):round(x_s[i] + 2)]
        elif round(z_s[i]) <= 1:
            square_3 = tiff_3d[0:2, round(y_s[i] - 1):round(y_s[i] + 2), round(x_s[i] - 1):round(x_s[i] + 2)]
        else:
            square_3 = tiff_3d[round(z_s[i] - 1):round(z_s[i] + 2), round(y_s[i] - 1):round(y_s[i] + 2),
                       round(x_s[i] - 1):round(x_s[i] + 2)]
        # ---------------------------------------------------------------
...
def get_pixels(tiff_3d, df_locs, offset, hyb, new_col_name):
    ...
    for i in range(df_locs.shape[0]):
        intensity = tiff_3d[
            np.round(z_s[i]).astype(np.int16), np.round(y_s[i]).astype(np.int16), np.round(x_s[i]).astype(np.int16)]
