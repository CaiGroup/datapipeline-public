...
class FMAligner:
    ...
    def __init__(self, ro, ref, ref_final=None):
        ...
        self.n_unmatch_give_up = 40
        self.min_dot_matches = 10
        self.n_longest_edges = 100
        self.set_n_longest_edges(500)
    ...
    def _find_ro_matching_pair(self, edge):
        ...
        u_r_max = 2048 - edge['y_dist'] + self.xyse
        v_r_min = edge['y_dist'] - self.xyse
        ...
        if float(edge['u_x']) < float(edge['v_x']):
            # u has a lower x than v, edge x distance is positive
            u_c_max = 2048 - edge['x_dist'] + self.xyse
            v_c_min = edge['x_dist'] - self.xyse

            ro_u_candidates = ro_u_candidates.iloc[list(ro_u_candidates['x'] < u_c_max)]
            ro_v_candidates = ro_v_candidates.iloc[list(ro_v_candidates['x'] > v_c_min)]

        else:
            # u has a larger x than v, x distance is negative
            u_c_min = -edge['x_dist'] - self.xyse
            v_c_max = 2048 + edge['x_dist'] + self.xyse

            ro_u_candidates = ro_u_candidates.iloc[list(ro_u_candidates['x'] > u_c_min)]
            ro_v_candidates = ro_v_candidates.iloc[list(ro_v_candidates['x'] < v_c_max)]
