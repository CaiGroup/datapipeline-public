...
from .helpers.combine_multi_dots import combine_locs
from .location_checks import get_location_checks
from .rand_list import are_jobs_finished, get_random_list
# Import some helpers
# --------------------------------------------------------------------------------
            ...
            if "Hyb" in sub_dir:
                ...
                if 'top' in self.dot_detection:
                    ...
                    list_cmd = [
                        ...
                        '--tiff_src', tiff_file_path, '--norm', self.normalization, '--channels',
                        self.decoding_individual,
                    ...
                    list_cmd = [
                        ...
                        '--offset2', offset[2], '--analysis_name', self.analysis_name, '--vis_dots',
                        self.visualize_dots,
                        '--back_subtract', self.background_subtraction, '--tiff_src', tiff_file_path, '--norm',
                        self.normalization,
                        ...
                        '--rolling_ball_kernel_size', self.rolling_ball_kernel_size, '--tophat_kernel_size',
                        self.tophat_kernel_size
                    ]
                    ...
                    list_cmd = [
                        'python', dot_detection_dir + '/adcg_2d.py', '--offset0', offset[0], '--offset1', offset[1],
                        '--offset2', offset[2],
                        '--analysis_name', self.analysis_name, '--vis_dots', self.visualize_dots, '--back_subtract',
                        self.background_subtraction,
                        '--tiff_src', tiff_file_path, '--norm', self.normalization, '--channels',
                        ...
                        self.gaussian_fitting,
                        '--radial_center', self.radial_center, '--strictness', self.strictness_dot_detection,
                        '--num_wav', self.num_wav,
                        '--z_slices', z_slice, '--min_weight_adcg', self.min_weight_adcg, '--final_loss_adcg',
                    ...
                    list_cmd = [
                        'python', dot_detection_dir + '/hist_jump_3d.py', '--offset0', offset[0], '--offset1',
                        offset[1],
                        '--offset2', offset[2], '--analysis_name', self.analysis_name, '--vis_dots',
                        self.visualize_dots,
                        '--back_subtract', self.background_subtraction, '--tiff_src', tiff_file_path,
                        '--norm', self.normalization, '--channels', self.decoding_individual, '--chromatic',
                        self.chromatic_aberration,
                        '--rand', rand_dir, '--gaussian', self.gaussian_fitting, '--radial_center', self.radial_center,
                        '--strictness', self.strictness_dot_detection, '--num_wav', self.num_wav, '--z_slices', z_slice,
                        '--num_z', self.num_z, '--nbins', self.nbins, '--threshold', self.threshold, '--radius_step',
                        self.radius_step,
                        '--num_radii', self.num_radii, '--stack_z_s', self.stack_z_dots, '--back_blob_removal',
                        self.background_blob_removal,
                        '--rolling_ball', self.rolling_ball, '--tophat', self.tophat, '--blur', self.blur,
                        '--blur_kernel_size', self.blur_kernel_size, '--rolling_ball_kernel_size',
                        self.rolling_ball_kernel_size,
                        ...
                        '--tophat_raw_data', self.tophat_raw_data, '--tophat_raw_data_kernel',
                        self.tophat_raw_data_kernel_size,
