...
from datapipeline.dot_detection.helpers.background_subtraction import get_background, get_back_sub_check, \
    get_shifted_background, remove_blobs_with_masks_3d
...
from datapipeline.dot_detection.preprocessing.preprocess import get_preprocess_check, tophat_3d, blur_back_subtract_3d, \
    blur_3d, dilate_3d
...
def get_dots_for_tiff(tiff_src, offset=[0, 0, 0], analysis_name=None, bool_visualize_dots=False, \
                      bool_background_subtraction=False, channels_to_detect_dots=[0], bool_chromatic=False,
                      bool_gaussian_fitting=False, \
                      strictness=5, bool_radial_center=False, z_slices='all', num_wav=4, num_z=None, nbins=100,
                      dot_radius=1, threshold=.0001, \
                      radius_step=1, num_radii=2, bool_stack_z_dots=False, bool_blob_removal=False,
                      bool_rolling_ball=False, bool_tophat=False,
                      bool_tophat_raw_data=False, bool_blur=False, blur_kernel_size=5, rolling_ball_kernel_size=5,
                      tophat_kernel_size=100,
                      tophat_raw_data_kernel_size=0, overlap=.5, min_sigma=1, max_sigma=2, num_sigma=2,
                      bool_remove_bright_dots=True,
...
if __name__ == '__main__':
    ...
    if 'ipykernel' not in sys.argv[0]:
        ...
        if 'debug' not in sys.argv[1]:
            ...
            get_dots_for_tiff(
                tiff_src='/groups/CaiLab/personal/alinares/raw/2021_0512_mouse_hydrogel/HybCycle_12/MMStack_Pos0.ome.tif',
                offset=[0, 0, 0],
