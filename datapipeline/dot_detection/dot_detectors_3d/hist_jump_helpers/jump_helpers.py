...
def save_strictnesses(threshs, cumul_of_dots, thresh, df_dst):
    ...
    index_of_thresh = df[df.threshold == thresh].index

    if len(index_of_thresh) > 0:
        index_of_thresh = index_of_thresh[0]
        strictnesses = list(range(index_of_thresh - df.shape[0], index_of_thresh))
        strictnesses.reverse()
        strictnesses = - np.array(strictnesses)
    else:
        print(f'No dots found matching threshold of {thresh}')
        index_of_thresh = 0
        strictnesses = np.zeros(len(df), dtype=int)
...
def hist_jump_threshed_3d(tiff_3d, strictness, tiff_src, analysis_name, nbins, threshold, min_sigma, max_sigma,
                          num_sigma, overlap, bool_remove_bright_dots):
    """
...
if __name__ == '__main__':
    ...
    if sys.argv[1] == 'debug_jump_helper':
        ...
        threshold = .001
        dot_analysis = list(
            hist_jump_threshed_3d(tiff_3d, strictness, tiff_src, analysis_name, num_bins, dot_radius, threshold,
