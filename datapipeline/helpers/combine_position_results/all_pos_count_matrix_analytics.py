...
def get_plot_from_count_matrix_dict(all_pos_genes_per_cell, dst):
    ...
    plt.xticks(range(len(all_pos_genes_per_cell)), list(all_pos_genes_per_cell.keys()), rotation='vertical',
               fontsize=10)
    plt.tight_layout()
    plt.savefig(dst)
    # ------------------------------------------------
...
def get_all_count_matrix_src_s(analysis_dir):
    ...
    # ---------------------------------------------------------
    glob_me_for_count_matrics_indiv = os.path.join(analysis_dir, 'MMStack_Pos*', 'Segmentation', 'Channel*',
                                                   'count_matrix.csv')
