...
from .cellpose_segment import run_cellpose
from .cellpose_segment.helpers.combine_count_matrices import get_count_matrix_for_pos
from .post_processing.get_post_processing import save_labeled_img
# Segmentation Script
# ----------------------------
from .roi_segment import run_roi
...
class Segmentation:
    def __init__(self, data_dir, position, seg_dir, decoded_dir, locations_dir, barcode_dst, barcode_src,
                 bool_fake_barcodes, bool_decoding_individual, num_z_slices, seg_type, seg_data_dir, dimensions,
                 num_zslices,
    ...
    def run_segmentation_across(self):
        ...
        if 'roi' in self.seg_type:
            ...
            run_roi.run_me(self.seg_dir, decoded_genes_path, barcode_path, roi_zip_file_path, self.fake_barcodes,
                           self.num_zslices)
            # ----------------------------------------------

        elif "cellpose" in self.seg_type:
            run_cellpose.run_me(self.data_dir, self.seg_dir, decoded_genes_path, barcode_key_src, self.position,
                                self.labeled_img)
    ...
    def run_segmentation_individually(self):
        ...
        for channel_num in self.decoding_individual:
            ...
            if 'roi' in self.seg_type:
                ...
                run_cellpose.run_me(self.data_dir, segmented_dir, decoded_genes_path, barcode_key_src, self.position,
                                    self.labeled_img)
                # ----------------------------------------------
